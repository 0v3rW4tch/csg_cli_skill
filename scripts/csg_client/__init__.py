...
class CSGClient:
    ...
    def api_select_elec_bill_list(
        ...
        """
        Get electricity bill list for a given month.
        Args:
            area_code: Area code (e.g., "030000")
            year_month: Year and month in format "YYYYMM" (e.g., "202603")
            ele_customer_id: Electricity customer ID
        Returns:
            Electricity bill list data
        """
        path = "charge/selectElecBillList"
        payload = {
            JSON_KEY_AREA_CODE: area_code,
            "yearMonth": year_month,
            "eleCustIdList": [{"eleCustId": ele_customer_id}],
        }
        _, resp_data = self._make_request(path, payload)
        if resp_data[JSON_KEY_STA] == RESP_STA_SUCCESS:
            return resp_data[JSON_KEY_DATA]
    ...
    def get_elec_bill_list(
        self, account: CSGElectricityAccount, year_month: tuple[int, int]
    ) -> tuple[float | None, float | None]:
        """
        Get total charge and power consumption for a given month.
        Returns: (total_charge, total_power) in (CNY, kWh)
        """
        year, month = year_month
        resp_data = self.api_select_elec_bill_list(
            account.area_code,
            f"{year}{month:02d}",
            account.ele_customer_id,
        )
        bill = resp_data.get("billOverviewModelList", [{}])[0]
        total_charge = float(bill["totalElectricity"]) if bill.get("totalElectricity") else None
        total_power = float(bill["totalPower"]) if bill.get("totalPower") else None
        return total_charge, total_power
